                        ...
                        if player then
                                ...
                                if full_weight <= free_cap then
                                        ...
                                        if type(received_item) == 'number' and received_item == RETURNVALUE_NOERROR then
                                                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'You received {' .. add_item_name .. '} from Amber gift area.')
                                                db.query('DELETE FROM `z_ots_comunication` WHERE `id` = ' .. id)
                                                db.query('UPDATE `z_shop_history_item` SET `trans_state` = \'realized\', `trans_real` = ' .. os.time() .. ' WHERE id = ' .. id)
                                        else
                                                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, '[' .. add_item_name .. '] from Amber gift area is waiting for you. Please make place for this item in your backpack/hands and wait about ' .. SQL_interval .. ' seconds to get it.')
                                        end
                                else
                                        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, '[' .. add_item_name .. '] from Amber gift area is waiting for you. It weight is ' .. (full_weight / 100) .. ' oz., you have only ' .. (free_cap / 100) .. ' oz. free capacity. Put some items in depot and wait about ' .. SQL_interval .. ' seconds to get it.')
                                end
                        end
