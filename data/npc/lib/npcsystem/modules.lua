...
if Modules == nil then
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		local totalCost = 0
		local player = Player(cid)
		if(player:getStorageValue(10062) == 1) then
			totalCost = (amount * shopItem.buy) - ((amount * shopItem.buy)*0.25)
		elseif(player:getStorageValue(10061) == 1) then
			totalCost = (amount * shopItem.buy) - ((amount * shopItem.buy)*0.1)
		elseif(player:getStorageValue(10060) == 1) then
			totalCost = (amount * shopItem.buy) - ((amount * shopItem.buy)*0.08)
		elseif(player:getStorageValue(10059) == 1) then
			totalCost = (amount * shopItem.buy) - ((amount * shopItem.buy)*0.05)
		else
			totalCost = amount * shopItem.buy
		end
		-----------------------------------------
		if inBackpacks then
	...
	function ShopModule.requestTrade(cid, message, keywords, parameters, node)
		...
		for i = 1, #module.npcHandler.shopItems do
		    ...
		    if itemWindow[index+1].buy > 0 and player then
		    	if(player:getStorageValue(10062) == 1) then
			        itemWindow[index+1].buy = itemWindow[index+1].buy - itemWindow[index+1].buy*0.25 
				elseif(player:getStorageValue(10061) == 1) then
					itemWindow[index+1].buy = itemWindow[index+1].buy - itemWindow[index+1].buy*0.1 
				elseif(player:getStorageValue(10060) == 1) then
					itemWindow[index+1].buy = itemWindow[index+1].buy - itemWindow[index+1].buy*0.08
				elseif(player:getStorageValue(10059) == 1) then
					itemWindow[index+1].buy = itemWindow[index+1].buy - itemWindow[index+1].buy*0.05
				else
					itemWindow[index+1].buy = itemWindow[index+1].buy
		    	end
		    end
		end
		--------------------------------------------------------------------
