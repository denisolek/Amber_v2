...
function creatureSayCallback(cid, type, msg)    
    ...
    local isBarbarian = player:getStorageValue(10067)
    ...
    if(player:getVocation():getId() ~= 4 and player:getVocation():getId() ~= 8 and player:getVocation():getId() ~= 3 and player:getVocation():getId() ~= 7) then
        selfSay("The only vocation I talk to are knights and paladins.", cid)
        return false
    end

    if (msgcontains(msg, "help") and isBarbarian == -1) then
        selfSay("From me you can start barbarian outfit {task}.", cid)
    elseif (msgcontains(msg, "task") and (whichTask == -1 or whichTask==2)) then
        if isBarbarian == -1 then
            if taskStarted == -1 then
                selfSay("Kill 10 Erathals to accomplish this task. Do you accept?", cid)
                talkState[talkUser] = 2
            elseif taskStarted == 1 then
                selfSay("Did you kill 10 Erathals?", cid)
                talkState[talkUser] = 2
            else
                selfSay("You already did this task.", cid)
            end
        else
            selfSay("You are a barbarian ! I can't give you any more tasks.", cid)
        end
    ...
    elseif (msgcontains(msg, "yes") and talkState[talkUser] == 2) then
        if taskStarted == -1 then
            ...
            player:setStorageValue(6006, 2)
            player:setStorageValue(7022, 0)
            if(player:getStorageValue(7021) == -1) then
            ...
            if (player:getStorageValue(6005)+1 == 10) then
                ...
                player:setStorageValue(10067, 1)
                player:addOutfit(147)
                player:addOutfit(143)
    ...
    elseif (msgcontains(msg, "help") and isBarbarian == 1) then
        selfSay("Just say {first barbarian addon} or {second barbarian addon}.", cid)
    elseif (msgcontains(msg, "first barbarian addon")) then
        if(player:getStorageValue(10067) == -1) then
            selfSay("You are not a barbarian ! Make outfit first.", cid)
        elseif(player:getStorageValue(10068) == 1 or player:getStorageValue(10070) == 1) then
            selfSay("You already have this addon.", cid)
        else    
            selfSay("For first barbarian addon you will need faithkeeper. Do you have it with you?", cid)
            talkState[talkUser] = 5
        end
    elseif (msgcontains(msg, "yes") and talkState[talkUser] == 5) then
        if(player:getItemCount(7413) >= 1) then
            player:removeItem(7413,1)
            player:addOutfitAddon(147, 1)
            player:addOutfitAddon(143, 1)
            player:getPosition():sendMagicEffect(15)
            if(player:getStorageValue(10069) == 1) then
                player:setStorageValue(10069, -1)
                player:setStorageValue(10070, 1)
            else
                player:setStorageValue(10068, 1)
            end
    ...
    elseif (msgcontains(msg, "second barbarian addon")) then
        if(player:getStorageValue(10067) == -1) then
            selfSay("You are not a barbarian ! Make outfit first.", cid)
        elseif(player:getStorageValue(10069) == 1 or player:getStorageValue(10070) == 1) then
            selfSay("You already have this addon.", cid)
        else    
            selfSay("For second barbarian addon you will need Erathal boots. Do you have it with you?", cid)
            talkState[talkUser] = 6
        end
    elseif (msgcontains(msg, "yes") and talkState[talkUser] == 6) then
        if(player:getItemCount(7457) >= 1) then
            player:removeItem(7457,1)
            player:addOutfitAddon(147, 2)
            player:addOutfitAddon(143, 2)
            player:getPosition():sendMagicEffect(15)
            if(player:getStorageValue(10068) == 1) then
                player:setStorageValue(10068, -1)
                player:setStorageValue(10070, 1)
            else
                player:setStorageValue(10069, 1)
